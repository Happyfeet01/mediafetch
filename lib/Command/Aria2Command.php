    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($action === 'complete') {
            ...
            if ($context !== null && $path !== '') {
                ...
                $this->logger->debug(
                    'MediaFetch ignored an untracked aria2 completion hook.',
                    ['app' => 'mediafetch', 'gid' => $gid, 'path' => $path]
                );
            }
    ...
    private function getScanContext(array $row): ?array
    {
        ...
        if (isset($row['data']) && $row['data'] !== null && $row['data'] !== '') {
            try {
                ...
                $this->logger->warning(
                    'MediaFetch could not read the stored aria2 destination.',
                    ['app' => 'mediafetch', 'gid' => $row['gid'] ?? null]
                );
            }
        }
